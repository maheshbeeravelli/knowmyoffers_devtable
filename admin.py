                        ...
                        if e.user_type=="Admin":
                          ...
                          if database=="offer":
                            ...
                            offer        =   datamodel.Offers(store=store,title=title,offer_position=offer_position,
                                             offer_type=offer_type,aff_link=aff_link,description=description,expiry=expiry,
                                             category=category,sub_category=sub_category,ideal_for=ideal_for,enabled=enabled,
                                             editors_pick=editors_pick)
                            stores_update = db.GqlQuery("SELECT * FROM Stores WHERE store=:1",str(store))
                            
                            for store_update in stores_update:
                                if offer_type=="Coupon":
                                   store_update.coupons_count= store_update.coupons_count+1
                                else:
                                  store_update.deals_count = store_update.deals_count + 1
                                store_update.put()
                            if offer_type=="Coupon":
                            ...
                            query =  db.GqlQuery("SELECT * FROM Stores WHERE store=:1",store_name)
                            #datamodel.Stores.all(keys_only=True).filter('store', store_name)
                            entity = query.get()
                            if entity:
